...
module.exports = {
  ...
  initialize: async function ({ api, event, args, send, react }) {
    ...
    if (!input) {
      send(`To get started, type Pinterest followed by the name of the image you are looking for, and the expected number of images.\n\nExample:\n\npinterest soyeon - 10`);
    } else {
      try {
        const key = input.substr(0, input.indexOf('-')).trim();
        send(`Searching for ${key}`);
        const len = input.split("-").pop().trim() || 6;
        const data = await getPinterest(key);
        let num = 0;
        let file = [];
        for (let i = 0; i < parseInt(len); i++) {
          const imagePath = path.join(__dirname, `cache/${timestamp}_${i + 1}.jpg`);
          const download = (await axios.get(`${data[i]}`, { responseType: 'arraybuffer' })).data;
          fs.writeFileSync(imagePath, Buffer.from(download, 'utf-8'));
          file.push(fs.createReadStream(imagePath));
        }
        await api.sendMessage({ attachment: file, body: "" }, event.threadID, (err) => {
          if (err) {
            ...
            for (let i = 0; i < parseInt(len); i++) {
              fs.unlinkSync(path.join(__dirname, `cache/${timestamp}_${i + 1}.jpg`));
            }
          }
        }, event.messageID);
      } catch (error) {
